...
from inversion.loss import perc, mse, noise_reg
...
def project(
    ...
    cam_parameters = []
    for img in images:
        img.c.requires_grad = True
        cam_parameters.append(img.c)
    ...
    for step in pbar:
        ...
        for i in target_indices:
            ...
            ws = w_opt + w_noise
            synth_image = G.synthesis(ws, c=images[i].c, noise_mode='const')['image']
        ...
        if optimize_cam_step:
            ...
            current_cams = torch.cat([images[i].c for i in target_indices])
            original_cams = torch.cat([images[i].original_c for i in target_indices])
        ...
        if step % 25 == 0:
            for i in target_indices:
                with torch.no_grad():
                    synth_image = G.synthesis(w_opt, c=images[i].c, noise_mode='const')['image']
